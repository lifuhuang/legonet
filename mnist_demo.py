...
nn = NeuralNetwork(Adam(), 'logs', loss_fn='softmax_cross_entropy', target_dtype='float32')
nn.add(Input([28, 28, 1]))
nn.add(Convolution([3, 3], 512))
nn.add(Convolution([3, 3], 256))
nn.add(Pooling())
nn.add(FullyConnected(128, 'relu'))
...
if mode == 'train':
    ...
    print 'accuracy', (np.sum(np.argmax(Y_pred, axis=1) == np.argmax(Y_test, axis=1))) * 100.0 / X_test.shape[0]